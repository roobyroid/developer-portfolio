...
const CustomLink = ({ href, title, className = '' }) => {
	...
	return (
		<Link href={href} className={`${className} relative group `}>
			{title}
			<span
				className={`h-[1px] bg-dark inline-block absolute -bottom-0.5 left-0 group-hover:w-full transition-[width] ease duration-300
...
const CustomMobileLink = ({ href, title, className = '', toggle }) => {
	...
	return (
		...
		>
			{title}
			<span
				className={`h-[1px] bg-light inline-block absolute -bottom-0.5 left-0 group-hover:w-full transition-[width] ease duration-300
...
const NavBar = () => {
	...
	return (
		<header className="w-full px-32 py-8 font-medium flex justify-between items-center dark:text-light relative z-10 lg:px-16 md:px-12 sm:px-8">
			...
			bg-dark/90 dark:bg-light/75 rounded-lg backdrop-blur-sm py-32 "
				>
					...
					<nav className="flex items-center justify-center flex-wrap mt-2">
						<motion.a
							href="https://github.com/roobyroid"
							target="_blank"
